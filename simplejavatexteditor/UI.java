...
import static simplejavatexteditor.Application.readFileAsString;
...
public class UI extends JFrame implements ActionListener {
    ...
    private static JComboBox search_autocomplete;
    private JMenu menuFile;
    private JMenu menuEdit;
    private final JMenu menuFind, menuAbout, menuAutoCom;
    private final JMenuItem newFile, openFile, saveFile, close, cut, copy, paste, clearFile, selectAll, quickFind,
            aboutMe, aboutSoftware, wordWrap, _autoComplete;
    private final JToolBar mainToolbar;
    JButton newButton, openButton, saveButton, clearButton, quickButton, aboutMeButton, aboutButton, closeButton,
            boldButton, italicButton;
    private final Action selectAllAction;
    public static String main_str;
    ...
    public UI() {
        ...
        menuAutoCom = new JMenu("AutoComplete");
        //Font Settings menu
        ...
        _autoComplete = new JMenuItem("AutoComplete");
        //search_autocomplete = new JMenuItem("Complete Search AutoComplete",autocompleteIcon) ;
...
//        wordWrap.setToolTipText("Word Wrap");
        ...
        _autoComplete.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent ae) {
                main_str = textArea.getSelectedText();
                String data;
                try {
                    data = readFileAsString("C:\\Users\\Harshil Mehta\\Documents\\NetBeansProjects\\Simple-Java-Text-Editor-master\\data.txt");
                    String allWords[] = data.split(", ");
                    Application app1 = new Application(main_str);
                    ArrayList<String> myMatches = app1.run(allWords);
                    search_autocomplete = new JComboBox();
                    for (String myMatche : myMatches) {
                        search_autocomplete.addItem(myMatche);
                    }
                    mainToolbar.addSeparator();
                    search_autocomplete.setMaximumSize(new Dimension(100, 30));
                    mainToolbar.add(search_autocomplete);
                    mainToolbar.addSeparator();
                } catch (Exception ex) {
    ...
    protected void processWindowEvent(WindowEvent e) {
        if (e.getID() == WindowEvent.WINDOW_CLOSING) {
            if (edit) {
                Object[] options = {"Save and exit", "No don't Save and exit", "Return"};
    ...
    private void saveFile() {
        ...
        if (option == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                try (BufferedWriter out = new BufferedWriter(new FileWriter(openFile.getPath()))) {
                    out.write(textArea.getText());
                }

                enableAutoComplete(openFile);
                edit = false;
            } catch (IOException ex) { // again, catch any exceptions and...
                // ...write to the debug console
                System.err.println(ex.getMessage());
            }
        }
                ...
                for (int i = 0; i < flavors.length; i++) {
                    if (flavors[i].isFlavorJavaFileListType()) {
                        ...
                        try {
                            ...
                            for (String dragDropExtensionFilter1 : dragDropExtensionFilter) {
                                if (fileName.endsWith(dragDropExtensionFilter1)) {
